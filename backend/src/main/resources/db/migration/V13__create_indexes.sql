...
CREATE INDEX idx_tasks_overdue ON tasks(user_id, due_date) WHERE is_completed = FALSE AND due_date < CURRENT_TIMESTAMP;
...
CREATE INDEX idx_file_attachments_pending_scan ON file_attachments(virus_scan_status) WHERE virus_scan_status = 'PENDING';

-- Covering index for notification list queries
CREATE INDEX idx_notifications_user_unread_created ON notifications(user_id, created_at DESC)
    WHERE is_read = FALSE
    INCLUDE (notification_type, title, message, task_id);
